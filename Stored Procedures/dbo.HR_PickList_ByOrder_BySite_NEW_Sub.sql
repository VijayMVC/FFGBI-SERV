...
-- exec [dbo].[HR_PickList_ByOrder_BySite_NEW_Sub] 132603,2,2,0,1


CREATE PROCEDURE [dbo].[HR_PickList_ByOrder_BySite_NEW_Sub] 
@OrderNo Int,
@SubOrderNo Int,
@SiteID int,
@Addon int,
...
BEGIN
	...
	Select	
			...
			pr.name,
			pr.inventoryid,
			CASE  when isNull(OA.InProductionStock,0) = 1 then 'PRODUCTION!' ELSE INV.[Name] end AS [Pallet Inventory], --added by KH
			CASE  when isNull(OA.InProductionStock,0) = 1 then 'PRODUCTION!' ELSE INVL.Code end AS [Pallet Location], --added by KH
			PRL.Code,
			os.Site_Comments
	...
	left join dbo.HR_Order_Allocation OA (nolock) on OA.HR_BoxNo = OS.Box_No and OS.Group_Order_No = OA.OrderNo
		and os.Sub_Order_no=oa.SubOrderNo
...
--Added by Martin McGinn 26/11/2018 for TTS Orders
		and 
			os.Sub_Order_no=@SubOrderNo
		and 
	...
	group by pal.palletnumber, OA.DNOB, OA.USEBY,OA.SiteID,PalletNo,OA.OrderNo,OA.SubOrderNo,OA.Modified,OS.Modified,OS.Sub_Order_no,OA.OrderDate, OA.ProductCode,OA.InProductionStock,OA.HR_BoxNo,OA.PalletID,
			 ...
			 INV.[Name], INVL.Code, INV.description3,pk.prday --,boxno
	...
	Select 
			...
			VIP,
			--Comments,
			'Pack Date ' + CONVERT(nvarchar(10), IsNull(prday,''), 3) + ', ' +IsNull(Comments,'') As 'Comments',
			totalOrdered,
			Description3,
			[Name],
			inventoryid,
	...
	where	not(siteid = 'FH' and isnull(inventoryid,0) In(23,64,63,66,65,67)) 
			and not(siteid = 'FGl' and isnull(inventoryid,0) In(75,101,102)) 
			and not(siteid = 'FO' and isnull(inventoryid,0) In(105,120,121,131))
			and not(siteid = 'FD' and isnull(inventoryid,0) In(83,84,85))
			and not(siteid = 'FC' and isnull(inventoryid,0) In(165,166))
	...
	where	--OSSuborderNo = 1 --main picklist criteria
			--and 
			--OAModified <>1  --main picklist criteria
			--and 
			PRDescription3 = 'SAWYERS' 

	Group By OrderNo, OASuborderNo,OAModified,OSModified,OSSuborderNo,OAOrderDate,ProductCode,PalletNo,Product_Name,Siteid,DNOB,UseBy,ProductionStock,HR_BoxNo,
			 ordered_qty,VIP,Comments,totalOrdered,Description3,[Name],Code,inventoryid,Site_Comments, [Pallet Inventory], [Pallet Location]
			 , PRDescription3, prday--, boxno
