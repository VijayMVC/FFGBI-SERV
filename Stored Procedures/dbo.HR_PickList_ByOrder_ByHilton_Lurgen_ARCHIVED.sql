...
-- =============================================
--exec [dbo].[HR_PickList_ByOrder_ByHilton_Lurgen] 116323
CREATE PROCEDURE [dbo].[HR_PickList_ByOrder_ByHilton_Lurgen_ARCHIVED] 
@OrderNo Int
--@SiteID int
AS
BEGIN
...
and oa.siteid = 3--@SiteID
group by OA.DNOB, OA.USEBY,OA.SiteID,PalletNo,OA.OrderNo,OA.SubOrderNo,OA.ProductCode,OA.InProductionStock,OA.HR_BoxNo,OA.PalletID,OS.VIP,OS.Comments,OS.Ordered_Qty,OS.Product_Name,PT.[Description3],pr.name,PRL.Code,StorageType,pr.inventoryid,PR.description3
order by VIP,PT.[Description3],HR_Boxno, OA.USEBY, OA.DNOB,pr.name


Select 
OrderNo, SubOrderNo,ProductCode,PalletNo,Product_Name,Siteid,DNOB,UseBy,ProductionStock,HR_BoxNo,ordered_qty,Sum(QTY) As Qty,VIP,Comments,totalOrdered,Description3,Name,inventoryid,Code,PRdes3
From #tmp
where siteid = 'FH' and isnull(inventoryid,0) =23 
Group By 
OrderNo, SubOrderNo,ProductCode,PalletNo,Product_Name,Siteid,DNOB,UseBy,ProductionStock,HR_BoxNo,ordered_qty,VIP,Comments,totalOrdered,Description3,Name,inventoryid,Code, PRdes3
Order By
