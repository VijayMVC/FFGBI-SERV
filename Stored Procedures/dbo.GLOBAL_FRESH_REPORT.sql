...
-- =============================================
CREATE PROCEDURE [dbo].[GLOBAL_FRESH_REPORT] 
...
FROM         Stock_Packs AS pk WITH (nolock) INNER JOIN
					Sites S WITH (nolock) on pk.SiteID = S.SiteID  INNER JOIN
                      ...
                      left join [Currency Exchange] CE with (nolock) on pk.siteid = CE.siteid and startingdate =(select max([StartingDate]) from [Currency Exchange] where CE.siteid = pk.siteid )  
                      Inner JOIN
...
mat1.storageType = 1 and pr.InventoryTypeID in (1,3) and not(pr.description1 like '%FROZEN%')
  --and ((getdate() between valuefrom and valueto) or (getdate()-2.5 between valuefrom and valueto))
