...
-- 4 is FD  2 is FO
-- exec [dbo].[HR_PickList_ByOrder_BySite_NEW_Alt] 123823,3,1,0


CREATE PROCEDURE [dbo].[HR_PickList_ByOrder_BySite_NEW_Alt] 
@OrderNo Int,
@SiteID int,
@Addon int,
@Sawyers int
...
--Select * From #t4
	...
	where	not(t.siteid = 'FH' and isnull(inventoryid,0) In(23,64,63,66,65,67)) 
			and not(t.siteid = 'FGl' and isnull(inventoryid,0) In(75,101,102)) 
			and not(t.siteid = 'FO' and isnull(inventoryid,0) In(105,120,121,131))
			and not(t.siteid = 'FD' and isnull(inventoryid,0) In(83,84,85))
			and not(t.siteid = 'FC' and isnull(inventoryid,0) In(165,166))
