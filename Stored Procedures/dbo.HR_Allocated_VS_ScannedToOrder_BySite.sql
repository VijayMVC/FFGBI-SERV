...
-- Description:	allocated VS scanned
-- =============================================

--exec [dbo].[HR_Allocated_VS_ScannedToOrder_BySite] 104883,5
CREATE PROCEDURE [dbo].[HR_Allocated_VS_ScannedToOrder_BySite] 
@OrderNo bigint,
@siteid int
AS
BEGIN
...
insert into #TempOrder select ''''+[Order No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Shipment Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FC'
insert into #TempOrder select ''''+[Order No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Shipment Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FO'
insert into #TempOrder select ''''+[Order No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Shipment Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FH'
insert into #TempOrder select ''''+[Order No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Shipment Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FGL'
insert into #TempOrder select ''''+[Order No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Shipment Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FD'

insert into #TempOrder select ''''+[No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FC'
insert into #TempOrder select ''''+[No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Header] where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FO'
insert into #TempOrder select ''''+[No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Header]where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FH'
insert into #TempOrder select ''''+[No_]+'''' from [FFGSQL01].[FFG-Production].dbo.[FFG LIVE$Sales Header]where [External Document No_] like @NAVOrder and [Shortcut Dimension 1 Code] = 'FGL'
...
left join [FFGSQL06].FFGProductionPlan.dbo.HR_Product_Subs PS on right(TP.productcode,3) = cast(PS.Boxno as bigint)
...
into #TMP2
from #TMP TP
where TP.[type] = 'Innova'  and TP.[site] = @Siteid
group by TP.orderno,TP.ProductCode, TP.DNOB, TP.Useby, TP.[Site],qty,Tp.[type],TP.[Priority]
...
and P1.storagetype =1 and TP.[site] = @Siteid
