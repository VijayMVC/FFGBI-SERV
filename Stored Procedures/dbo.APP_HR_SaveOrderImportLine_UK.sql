...
-- Description:	Records a HR UK Import Line - Based on Tommy ETL
-- =============================================
CREATE PROCEDURE [dbo].[APP_HR_SaveOrderImportLine_UK] 
...
BEGIN
	...
	-- Insert Values
    INSERT 
		INTO HR_Order_Summary_Import_UK(
		[RowNo],
	...
	update
		[FFG_DW].[dbo].[HR_Order_Summary_import_UK] 
    set 
	...
	set 
		OS.box_no = BL.boxNo
    from
		[FFG_DW].[dbo].[HR_Order_Summary_import_UK] OS 
...
--	From [FFG_DW].[dbo].[HR_Order_Summary_import_UK] h
--	Left Join #b b On h.Box_No=b.BoxNo And b.FFGSite='UK'
--	Where h.Ordered_Qty >0 And h.RowNo=@RowNo

--	Update [FFG_DW].[dbo].[HR_Order_Summary_import_UK] Set Ordered_Qty=IsNull(EstCases,0) Where RowNo=@RowNo
