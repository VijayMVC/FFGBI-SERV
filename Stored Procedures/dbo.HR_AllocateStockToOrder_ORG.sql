...
-- Description:	org script before tommy changed it.
-- =============================================
CREATE PROCEDURE [dbo].[HR_AllocateStockToOrder_ORG]
	@OrderNo Bigint,
	@OrderSubNo int,
	@BoxNo nvarchar(20)
AS
...
FROM	HR_Order_Summary_import AS HR  INNER JOIN
 [FFGSQL06].[FFGProductionPlan].[dbo].HR_Product_Subs AS HPS  ON HR.Box_No = HPS.BoxNo INNER JOIN
...
and pk.DNOB <= (HR.Delivery_Date + 1) and pk.UseBy >= (HR.Delivery_Date + 5)
and NOT EXISTS (select * FROM [FFG_DW].[dbo].[HR_Order_Allocation] oA3 where oA3.BoxNo  = pk.SSCC and [OrderNo] = @OrderNo and oA3.[SiteID] = pk.SiteID)
order by datediff("d", @Today, pk.UseBy),HR.Box_No
