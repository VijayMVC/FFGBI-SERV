...
-- Description:	Stored procedure for all frozen
 ...
 -- THIS IS NOT THE REPORT USED BY THE GROUP FROZEN REPORT - SP_FROZENBASE_TEST is the SP that is used by that report

-- =============================================
--exec [dbo].[SP_FrozenBase]  'mat1.ProductCode = ''912313231'' ' 
CREATE PROCEDURE [dbo].[SP_FrozenBase_ARCHIVED] 
@_Where nvarchar(max)
AS
BEGIN
...
and pk.weight > 0.1'--  and mat1.productcode = 901910993'
+ @_Where
exec(@SQL)

END
--exec [dbo].[SP_FrozenBase] '' 
GO
