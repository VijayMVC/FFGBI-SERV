...
-- =============================================
--exec [Group_Frozen_Weekly_Snapshot_Report_daily]
CREATE PROCEDURE [dbo].[Group_Frozen_Weekly_Snapshot_Report_daily] 
...
from		
	...
	left join Stock_Packs sp (nolock) on sp.ProductCode = p.ProductCode
...
select distinct tt.ProductCode,-- tt.name, [Customer Brand],Product,t.Description3,t.Dimension1,sum(isnull(t.[weight],0)) as [weight], avg(isnull(PricePerKg,0)),sum(isnull(convertedTotalValue,0))as convertedTotalValue,[date],
	...
	-- filters out the Petfood
	when pt.Description3 = '103 - OFFAL' and p.Dimension1 IN (30) and p.Dimension2 IN (32) or pt.ProductTypeID = 177 or p.productcode in (300610746,353210746) and not(p.ProductCode in(93011633)) then 'PET FOOD'
