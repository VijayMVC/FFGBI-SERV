...
-- Description:	report used by tesco team to confirm dates to hilton
-- =============================================
-- exec [DROG/OCADO_ConfirmDate_report_DNOB] 125243
CREATE PROCEDURE [dbo].[DROG/OCADO_ConfirmDate_report_DNOB]
@OrderNo Int
AS
BEGIN
...
OS.VIP,OS.Comments,
(select sum(cast(Ordered_Qty as int)) as TotalOrdered from HR_Order_Summary_Import where Group_Order_no = OA.OrderNo and Sub_Order_no = OA.SubOrderNo and (box_no is not null and round(Ordered_Qty,0) is not null)) as totalOrdered
,--PT.[Description3],
...
--select * from #tmp where [HR_BoxNo]='725' --and SubOrderNo='2'

Select distinct 
TP.OrderNo,TP.SubOrderNo,right(TP.ProductCode,3) as boxno,
(select Ordered_Qty from dbo.HR_Order_Summary_import where Group_Order_No =TP.OrderNo and Box_No = TP.HR_BoxNo and Sub_Order_no = TP.SubOrderNo ) as Ordered_Qty ,
(select Product_Name from dbo.HR_Order_Summary_import where Box_No =right(TP.ProductCode,3) and Group_Order_No =TP.OrderNo and Sub_Order_no = TP.SubOrderNo ) as ProductName,
(select Sum(QTY)as confiredQty from #tmp where Orderno = TP.OrderNo  and SubOrderNo = TP.SubOrderNo  and ProductCode =TP.ProductCode and DNOB =TP.DNOB and UseBy= TP.UseBy and siteid =tp.siteid) as confiredQty,
...
'DNOB' as [type]
From #tmp TP
where  TP.DNOB >= (TP.Delivery_Date + 0.5) --OR TP.UseBy <= (TP.Delivery_Date + 5.5)
Group By 
TP.DNOB,TP.UseBy,TP.OrderNo,TP.ProductCode,TP.ordered_qty,TP.[Description3],TP.QTY,Product_Name,TP.HR_BoxNo,Siteid,TP.SubOrderNo
...
Select distinct 
TP.OrderNo,TP.SubOrderNo,right(TP.ProductCode,3) as boxno,
(select Ordered_Qty from dbo.HR_Order_Summary_import where Group_Order_No =TP.OrderNo and Sub_Order_no = TP.SubOrderNo  and Box_No = TP.HR_BoxNo) as Ordered_Qty ,
(select Product_Name from dbo.HR_Order_Summary_import where Box_No =right(TP.ProductCode,3) and Group_Order_No =TP.OrderNo and Sub_Order_no = TP.SubOrderNo ) as ProductName,
(select Sum(QTY)as confiredQty from #tmp where Orderno = TP.OrderNo and SubOrderno = TP.SubOrderNo  and ProductCode =TP.ProductCode and DNOB =TP.DNOB and UseBy= TP.UseBy and siteid =tp.siteid) as confiredQty,
