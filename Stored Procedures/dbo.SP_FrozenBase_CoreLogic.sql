...
-- =============================================
--exec [dbo].[SP_FrozenBase_CoreLogic]  '1,2,3,4,5'

-- THIS CURRENTLY IS NOT BEING USED - IT SHOULD BE IN FUTURE BUT THE STOCK SHEETS USE THS *BAD* DUNAMIC SQL QUERY FOR GOOD REASON

CREATE PROCEDURE [dbo].[SP_FrozenBase_CoreLogic] 
	-- The Where Logic Will be handled ina  different SP
	@_SiteID nvarchar(max)
AS
BEGIN
...
set 
	@MaxValueto = (select max([ValueTo]) from Product_Price)

set 
	@Today = (SELECT DATEADD(day, DATEDIFF(day, 0, getdate()), 0))

set 
	@Campsiedate = (select max([StartingDate]) from [Currency Exchange] CE where CE.siteid = 1  )

set 
	@OmaghDate =(select max([StartingDate]) from [Currency Exchange] CE where CE.siteid = 2 )

set 
	@HiltonDate =(select max([StartingDate]) from [Currency Exchange] CE where CE.siteid = 3 )

set 
	@DonegalDate =(select max([StartingDate]) from [Currency Exchange] CE where CE.siteid = 4 )

set 
	@GloucesterDate =(select max([StartingDate]) from [Currency Exchange] CE where CE.siteid = 5  )

set 
	@MeltonDate = (select max([StartingDate]) from [Currency Exchange] CE where CE.siteid = 6  )
...
SELECT distinct 
	...
	CASE 
		WHEN 
			len(mat1.ProductCode) <> 9 
		THEN 
			'x'+RIGHT(mat1.ProductCode, 3) 
		ELSE 
			isnull(mat1.ProductCode, 'NONE') 
	END AS BaseProd, 
	...
	case 
		when 
			DATEDIFF(d, pk.prday, @Today) >= 0 
			and  
			DATEDIFF(d, pk.prday, @Today) <= 7 
		then 
			DATEDIFF(d, pk.prday, @Today) 
		else null 
	end as Age0to7,
					  	  
	case 
		when 
			DATEDIFF(d, pk.prday, @Today) >= 8 
			and  
			DATEDIFF(d, pk.prday, @Today) <= 180 
		then 
			DATEDIFF(d, pk.prday, @Today) 
		else 
			null 
		end as Age1to6Months,
					  
	case 
		when 
			DATEDIFF(d, pk.prday, @Today) >= 181 
			and  
			DATEDIFF(d, pk.prday, @Today) <= 365 
		then 
			DATEDIFF(d, pk.prday, @Today) 
		else 
			null 
		end as Age6to12Months,
					  
	case 
		when 
			DATEDIFF(d, pk.prday, @Today) >= 366 
			and  
			DATEDIFF(d, pk.prday, @Today) <= 730 
		then 
			DATEDIFF(d, pk.prday, @Today) 
		else 
			null 
	...
	case 
		when 
			DATEDIFF(d, pk.prday, @Today) > 731 
		then 
			DATEDIFF(d, pk.prday, @Today) 
		else 
			null 
		end as AgeGthan2Years,                   
	...
	case 
		when pk.siteid = 1 then @CampsieDate
	...
	(select top 1 [Exchange Rate Amount] 
		from [FFGSQL01].[FFG-Production].DBO.[FFG LIVE$Currency Exchange Rate] 
		where [Currency Code]='EUR' 
		order by [Starting Date] desc) AS [ExchangeRateAmount],
	...
	--Filters out Commercial Primal
		when (
			...
			and 
			mat1.ProductCode not in (301510640,301510426) 
		) 
		or 
		(
			mat1.ProductCode in 
	...
	-- filters out the 70vl
	when
		(mat1.ProductCode in (
			-- DBogues 2016 09 20 : E-mail from Stephen McCormick from Paul Armstrong
			710610584
		))
		then '70 VL'

	-- filters out the 75vl
	when
		(mat1.ProductCode in (
			-- DBogues 2016 09 20 : E-mail from Stephen McCormick from Paul Armstrong
			633221604
	...
	when 
		(pt.ProductTypeID = 114 and mat1.Dimension1 IN (10,15,27,30,33,37))
		OR
		(mat1.ProductCode in(
			-- DBogues 2016 09 20 : E-mail from Stephen McCormick from Paul Armstrong
			743213619
	...
	when 
		(pt.ProductTypeID = 113 and mat1.Dimension1 IN (15,27,30,33,37,86))
		OR
		(mat1.ProductCode in(
			-- DBogues 2016 09 20 : E-mail from Stephen McCormick from Paul Armstrong
			710610545, 710610550, 633221650
			)
		) then '95 VL'
	

	-- filters out the 98VL
	when pt.ProductTypeID = 112 and mat1.Dimension1 IN (15,27,30,33,37,86) 
		or (mat1.ProductCode in(830123531,670110531)) then '98 VL'
	...
	-- DBogues - 2016 08 15 - Ticket 4205 : This is a crap way of doing this but these reports are meant to be getting redeveloped in coming months 
	when (mat1.Dimension1 = 30 and mat1.ProductCode = 305710851) then 'OFFAL'
...
-- This removes the need for the @Where which was the cause for the dynamic sql
and 
  pk.SiteId in (select val from dbo.f_split(@_SiteID, ','))




/*
-- Use this logic for torubleshooting particular products
and 
	mat1.productcode in (	
		633221604
	)
