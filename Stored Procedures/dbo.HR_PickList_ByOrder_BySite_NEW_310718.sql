...
-- exec [dbo].[HR_PickList_ByOrder_BySite_NEW_310718] 129587,4,0,0


CREATE PROCEDURE [dbo].[HR_PickList_ByOrder_BySite_NEW_310718] 
@OrderNo Int,
@SiteID int,
@Addon int,
@Sawyers int
...
BEGIN
	...
	Select 
			...
			Sum(QTY) As Qty,
			VIP,
			'Pack : ' + CONVERT(nvarchar(10), IsNull(prday,''), 3) + ' : ' +IsNull(Comments,'') As 'Comments',
			totalOrdered,
			Description3,
			[Name],
			inventoryid,
	...
	where	not(siteid = 'FH' and isnull(inventoryid,0) In(23,64)) 
			and not(siteid = 'FG' and isnull(inventoryid,0) In(75,101)) 
			and not(siteid = 'FO' and isnull(inventoryid,0) In(105,120,121))
