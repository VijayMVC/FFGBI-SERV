...
--exec [SP_GetLotListForReports_KH] '01/25/16', '01/25/16', 0

CREATE procedure [dbo].[SP_GetLotListForReports_KH]
@BeginDate datetime,
@EndDate datetime,
@LotsTypes int
...
--print 'Boning'
	...
	FROM         	
		...
		INNER JOIN proc_lots WITH (nolock) ON x.tolot = proc_lots.lot 
		inner join proc_xactpaths pxp with (nolock) on x.xactpath = pxp.xactpath 
	WHERE  
	...
	FROM 
		...
		INNER JOIN proc_lots WITH (nolock) ON proc_packs.lot = proc_lots.lot 
	WHERE  
