...
-- =============================================
--exec [dbo].[SP_FrozenBase_Test] '' 'and pk.siteid=5' 
Create PROCEDURE [dbo].[ARCHIVED_SP_FrozenBase_Test_DB_BackUP_20160906] 
	@_Where nvarchar(max)
AS
BEGIN
...
case 
	...
	when (pt.Description3 = ''101 - PRIMAL'' and mat1.Dimension1 IN (30,27,45,63,35) and mat1.ProductCode not in (301510640,301510426) ) or (mat1.ProductCode in (114823472,155210261,155210323,155210324,860110077,155110338,155210228))  then ''COMMERCIAL PRIMAL'' 	
	...
	when pt.Description3 = ''101 - PRIMAL'' and (con.Name = ''GREENCORE'' or con.Name = ''SAMWORTH BROS'') or mat1.ProductCode in (881910529,881510683,881910659,301510640,301510426) then ''PRIMAL INGREDIENTS''
	
	--filters out the VL Ingredients
	when (pt.Description3 = ''102 - VL'' and mat1.Dimension1 in (13,83)) or (mat1.Dimension1 IN (27) and pt.Description3 = ''104 - MINCE'') then ''VL INGREDIENTS''
	
	--filters out the Flank
	when pt.ProductTypeID = 118 and mat1.Dimension1 IN (27,30,33,37,86) and not (mat1.ProductCode in(330610740,300110615,300110616,305110611,305110615,305210613,305810613,305810614,7160610601)) then ''FLANK''
	
	--filter ou the Chinese Flank	
	when mat1.Dimension1 IN (30,93) AND pt.Description3 = ''102 - VL'' and  pt.ProductTypeID <> 122 and not (mat1.ProductCode in(300610850,935710563,935710600,930110868,305710851)) then ''CHINESE FLANK''
	...
	-- filter out Birdseye
	when con.Name = ''BIRDSEYE'' and mat1.Dimension1 IN (98) then ''BIRDSEYE''
...
and pk.weight > 0.1 '--and mat1.productcode = '
+ @_Where
exec(@SQL)
