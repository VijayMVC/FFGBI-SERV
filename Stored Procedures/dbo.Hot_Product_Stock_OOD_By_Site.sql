...
-- Description:	Hot Product report with Out of date product by site
-- =============================================
CREATE PROCEDURE [dbo].[Hot_Product_Stock_OOD_By_Site] 
@_site int
AS
BEGIN
...
DATEDIFF(d, pk.prday, @Today) AS Age, 
 ...
 --case when DATEDIFF(d, @Today, pk.UseBy ) >= 0 and  DATEDIFF(d, @Today, pk.UseBy) <= 7 then DATEDIFF(d, @Today, pk.UseBy) else null end as Age0to7,
...
mat1.[DaysTillExpire],
con.[priority],
datediff("d", @Today, pk.UseBy) as daysleft
