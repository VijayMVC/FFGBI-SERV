...
-- 4 is FD  2 is FO
-- exec [dbo].[HR_PickList_ByOrder_BySite_NEW_Alt] 123643,2,0,0


create PROCEDURE [dbo].[HR_PickList_ByOrder_BySite_NEW_Alt-backup] 
@OrderNo Int,
@SiteID int,
@Addon int,
@Sawyers int
...
SELECT      [Item No_] ProductCode, Sum([Remaining Case Qty_]) Qty,  [Location Code] SiteID, [Pallet No_] PalletNo, [Kill Date] KillDate, Min([Pack Date]) PackDate, DNOB,  [Use by Date] UseBy, --[Innova Lot No_] LotNo,
...
--AND [Item No_]='501910642' AND [Location Code]='FD' --AND [Use by Date]='11/17/2017'

Group By [Item No_],[Location Code], [Pallet No_], [Kill Date],  DNOB, --[Innova Lot No_], [Pack Date],
...
SELECT 
   SS.SiteID, SS.ProductCode,SS.KillDate, SS.DNOB, SS.UseBy, --SS.LotNo,SS.PackDate, 
	   STUFF((SELECT '/ ' + US.ALT
			  FROM #t3 US
...
Into #t4
FROM #t2 SS
GROUP BY SS.ProductCode, SS.SiteID,SS.KillDate,  SS.DNOB, SS.UseBy--, SS.LotNo  SS.PackDate,
ORDER BY 1
