...
--exec  dbo.[SP_FreshStockVsSalesPerson] '1,2,3,4,5,6,7' , 'Clerice Auzende,Claire Murphy,Colin Maxwell,Christopher Snow,Commercial Team,David Campbell,Fintan Kelly,Jacqueline Phair,Marie Dibartolo,Nicola Wilson,Philip Robinson,Simon Byrne,Shaun Campbell,Stephen Dunn'
CREATE PROCEDURE [dbo].[SP_FreshStockVsSalesPerson]
...
SELECT     distinct pk.id,pk.OrgSite,os.orgsitename,sscc, pk.FixedCode, SP.SalesPerson,
pk.DeviceID, 
...
when pk.siteid = 4 then 'FD'					---site ID's
...
--(pk.[Weight]*isnull(PP.value,(select top 1 value from Product_Price where ProductCode = mat1.ProductCode and pk.siteid = pp.siteid and [ValueTo] = (select max([ValueTo]) from Product_Price where ProductCode = mat1.ProductCode and pk.siteid = pp.siteid))) ) AS [Value],
...
LEFT JOIN [FFGSQL03].[FFGData].[dbo].[SalesPersonVsProductGrouping] AS SP WITH (NOLOCK) ON SP.[Grouping] = con.[Name] AND SP.[Fresh/Frozen] = 'Fresh'
